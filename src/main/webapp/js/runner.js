var TheatreSysLib = TheatreSysLib || {};

//By default show table on clicking the to navbar buttons
TheatreSysLib.showGrid = function(){
    ...
    xhr.open('get', '/TheatreSystem/rest/'+ me.dataUrl, false);
    xhr.send();
    ...
    me.gridButtons.forEach(button => {
        tableContent += `<a href = "${button.linkToJsp}"><button class="${button.cssClass}" id="${button.id}"><i class="fa ${button.fontAwesomeIcon}"></i> ${button.label}</button></a>`;
    });

   //beginning of creating the table html
   tableContent += `<br/><table class='${me.gridStyle}'><thead><tr>`;
    ...
    me.gridData.forEach(row => {
        ...
        tableContent += `<tr>`;
            ...
            /* adding the edit and delete buttons in each row for ${button.linkToJspwhich the unique button id created above will be append
    ...
    actionEvents.forEach(button => {
        ...
        document.getElementById(button.deleteBtnId).addEventListener('click', function(){
                ...
                TheatreSysLib.showGrid.call(me);
